  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        setLoading(false); // Asegurarse de que el loading se detiene al finalizar la carga
      }
    }
  ...
  return (
    <div className="max-w-6xl py-4 mx-auto sm:py-16 sm:px-24">
      ...
      <div className="relative">
        ...
        <Grid container spacing={4}>
          {loading ? (
            // Mostrar 3 Skeletons mientras se cargan los productos
            Array(3)
              .fill()
              .map((_, index) => (
                <Grid item xs={12} sm={6} md={4} key={index}>
            ...
            products
              .slice(currentIndex, currentIndex + itemsToShow) // Mostrar solo los productos actuales
              .map((product) => {
                const { attributes, id } = product;
                const { slug, images, productName } = attributes;
